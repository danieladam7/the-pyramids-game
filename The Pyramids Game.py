...
def verify_rule1(matrix):
    if matrix[0][4] == BLUE:
        matrix[0][4] = randomize_color()
    if matrix[1][3] == BLUE:
        matrix[1][3] = randomize_color()
    if matrix[1][5] == BLUE:
        matrix[1][5] = randomize_color()
    if matrix[2][2] == BLUE:
        matrix[2][2] = randomize_color()
    if matrix[2][6] == BLUE:
        matrix[2][6] = randomize_color()
    if matrix[3][1] == BLUE:
        matrix[3][1] = randomize_color()
    if matrix[3][7]== BLUE:
        matrix[3][7] = randomize_color()
    for column in range(9):
        if matrix[4][column] == BLUE:
            matrix[4][column] = randomize_color()
...
def main():
    ...
    run = True
    start_ticks = pygame.time.get_ticks()  # start time for timeout. end game after 20 secs
    while run:
